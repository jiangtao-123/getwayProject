...
public class RtuModbusUtils {
	...
	private static RtuModbusUtils singleton;
	...
	private RtuModbusUtils() {
	}

	public static RtuModbusUtils getInstance() {
		if (singleton == null) {
			synchronized (RtuModbusUtils.class) {
				singleton = new RtuModbusUtils();
			}
	...
	public static void main(String[] args) throws JSONException {
		RtuModbusUtils.getInstance().start(true);
	}
		...
		if (isthread) {
			Thread thread = new Thread(new Runnable() {
				...
				public void run() {
					...
					// 初始化配置文件；
					RtuModbusUtils.getInstance().InitConfig();
					do {
						// 读取数据；
						RtuModbusUtils.getInstance().getModbusData();
	...
	public synchronized Hashtable getDataHashtable() {
		RtuModbusUtils.getInstance().setRead(true);// 加读锁；
		if (RtuModbusUtils.getInstance().write) {
			return null;
		} else {
			RtuModbusUtils.getInstance().setRead(false);// 释放读锁；
			// 删除本次缓存文件；
	...
	private void setModbusConfig() throws IOException, JSONException {
		...
		byte[] data;
		String json;
		stream = EdgeStudio.class.getResourceAsStream("/rtuMudbusConfig.json");
	...
	public synchronized void getModbusData() {
		if (RtuModbusUtils.getInstance().read) {
			return;
			// Thread.sleep(500);

		}
		// 写加锁
		RtuModbusUtils.getInstance().setWrite(true);
		if (modbusConfigList != null && modbusConfigList.size() > 0) {
			for (int i = 0; i < modbusConfigList.size(); i++) {
				...
				try {
					...
					transport.connect();
					// IModbus modbus = Modbus.newModbus(modbusConfig.slaveId,
					// transport, codec);
					byte[] data = null;
					...
					for (int i1 = 0; i1 < modbusConfig.areas.size(); i1++) {
						...
						IModbus modbus = Modbus.newModbus(area.slaveId, transport, codec);
						// 1:线圈 2:离散输入;3:保持寄存器;4:输入寄存器
						switch (area.area) {
						case 1:
							// 线圈
							data = modbus.ReadCoils(area.start, area.amount);
							// System.out.println(HexUtils.hexToInt(HexUtils.bytes2Hex(data))
							// + "><>PPPPPPP1");
							break;
						case 2:
							// 离散输入
						...
						case 4:
							...
							// System.out.println(HexUtils.bytes2Hex(data) +
							// "><>PPPPPPP4");
							break;
						}
					...
					transport = null;
					modbusConfig = null;
					RtuModbusUtils.getInstance().setWrite(false);
				}
		...
		// 写入缓存文件；
		writeFile();
		RtuModbusUtils.getInstance().setWrite(false);// 释放写锁；
	}
	...
	private void writeFile() {
		...
		try {
			...
			String str = parseToJson(RtuModbusUtils.getInstance().dataTable);
			FileUtils.creatFile(FileConstant.baseFileURI, FileConstant.rtuFileName + getDate(new Date()) + ".txt",
	...
	private void setIecData(byte slaveId, ArrayList list) {
		// TODO Auto-generated method stub
		if (this.checkData(((ArrayList) RtuModbusUtils.getInstance().getDataTable().get(slaveId + "")), list)) {
			RtuModbusUtils.getInstance().getDataTable().put(slaveId + "", list);
		}
	}
